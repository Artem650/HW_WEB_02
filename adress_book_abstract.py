...
class AddressBook(UserDict):
    ...
    def find_contact_by_name(self, contact_name):
        for key, value in self.data.items():
            if contact_name.title() == key:
                return print(value)

    def search_matches_in_addressbook(self, match):
        found_matches = []
...
class NoteBook(UserDict):
        ...
        if len(search_match) >= 1:
            ...
            for key, value in self.data.items():
                if search_match in str(value):
                    res += str(value) + " "
                    print(
                        f'Note "{search_match}" in {key} "{(str(value).split(" ")[0][0].upper() + str(value).split(" ")[0][1:] + str(value)[len(str(value).split(" ")[0]):])}" was found')
    ...
    def show_notebook(self):
        for name, note in self.data.items():
            print(
                f"{name}: {(str(note).split(' ')[0][0].upper() + str(note).split(' ')[0][1:] + str(note)[len(str(note).split(' ')[0]):])}")
